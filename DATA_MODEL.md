# 8BR — Competition Data Model

Authoritative documentation for the 8BR relational schema. It supports the legacy
**8BR** archive and all future 8BR competitions (Seasons, Cups, Tournaments)
...
7. **Provenance on everything.** Every primary entity has a `Provenance`
   (`IMPORTED_8BR` | `NATIVE_EGO`) and a nullable `legacyId`/`legacy*Id`. Imported
...
11. **CUID primary keys.** Stable, non-sequential string PKs are environment-portable
    (import/merge friendly) and uniform across the model. At 8BR scale (~2k
...
- **Season** (primary, recurring): `Competition(type=SEASON)` → optional `Division`s
  ...
  `Bracket` → optional `Stage(FINALS)`. Mirrors the 8BR season/division shape.
