# 8BR — 8 Ball Registry · PROJECT PLAN

**8BR — 8 Ball Registry**
<sub>formerly known as 8BR</sub>
...
| Item | Result |
|---|---|
| Working folder | `C:\Claude\8BR` (no nested 8BR folder) — *was `C:\Users\Cerebro\Documents\8BR`, deleted 2026-08-16* |
| Node.js | v24.18.0 |
| npm | 11.16.0 |
...
- **Primary title:** `8BR`
- **Full name:** `8 Ball Registry`
- **Legacy line:** `formerly known as 8BR` — treated as **secondary legacy text**, never part of the primary logo or main title.
...
8BR is the competitive organization and historical home for CueVerse online pool competition. It serves:
...
| Category | Role |
|---|---|
| **Seasons** | The **primary** recurring 8BR competition. |
...
- **History** contains the legacy **8BR archive** and must clearly distinguish **historical records** from **current 8BR competitions** (see §7 provenance).
...
**Rules:**
- The 8BR app **imports validated snapshots** into PostgreSQL; it never reads those CSVs as a live production source.
...
- **Provenance flag:** every record is marked as **imported historical (8BR legacy)** vs **native 8BR** so the two are always distinguishable in data and UI. Confidence/provenance signals from the source (`champion_confidence`, `bracket_reconstructed`, `source_kind`) are carried through, not dropped.
...
- **Phase 2 — Public archive (read-only):** Seasons/Cups/Tournaments, Standings, Players, History (8BR archive), Hall of Fame, Rules, News; search/filter; CSV/JSON exports.
...
6. **Live vs. historical scope timing** — when native 8BR competitions begin being entered alongside the imported archive.
...
- Work only inside the project root (`C:\Claude\8BR`); no nested 8BR folder.
