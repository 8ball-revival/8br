...
try {
  ...
  const bad = await transitionCupState(actor, id, 'IN_PROGRESS')
  ...
  const noBracket = await transitionCupState(actor, id, 'COMPLETED')
  ...
  check('COMPLETED→CANCELLED rejected (terminal, no recovery)', !(await transitionCupState(actor,id,'CANCELLED')).ok)

  // Completed-lock: every normal mutation gate rejects server-side once COMPLETED.
  const lockScore = await requireCupState(id, ['IN_PROGRESS'])
  check('completed cup: scoring gate rejected (locked)', !lockScore.ok && /completed and locked/i.test(lockScore.error||''))
  const lockEntrants = await requireCupState(id, ['REGISTRATION_OPEN','REGISTRATION_CLOSED'])
  check('completed cup: entrant gate rejected (locked)', !lockEntrants.ok)
  const lockBracket = await requireCupState(id, ['REGISTRATION_CLOSED','BRACKET_GENERATED'])
