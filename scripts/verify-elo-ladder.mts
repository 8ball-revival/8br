...
const reg = (tid: number, username: string, playerId: string) => prisma.registration.create({ data: { tournamentId: tid, username, playerId, status: 'APPROVED' } })
...
async function ensurePlayers(ids: string[]) {
  for (const id of ids) await prisma.player.upsert({ where: { id }, update: {}, create: { id, primaryName: id, cueverseId: id } })
}
...
async function run() {
  ...
  await rebuild()
  let led = await prisma.ratingLedger.findMany({ where: { tournamentId: t1.id, playerId: 'A' }, orderBy: { sequence: 'asc' } })
  ...
  await rebuild()
  led = await prisma.ratingLedger.findMany({ where: { tournamentId: t2.id, playerId: 'A' } })
  ...
  const ledB = await prisma.ratingLedger.findMany({ where: { tournamentId: t2.id, playerId: 'B' } })
  ...
  await rebuild()
  const drawRow = await prisma.ratingLedger.findFirst({ where: { tournamentId: t3.id, playerId: 'A' } })
  ...
  const teamA = await prisma.tournamentTeam.create({ data: { tournamentId: tt.id, registrationId: teamAReg.id, name: 'Alpha', members: { create: [{ name: 'T1a', playerId: 'T1a', memberOrder: 0 }, { name: 'T1b', playerId: 'T1b', memberOrder: 1 }] } } })
  const teamB = await prisma.tournamentTeam.create({ data: { tournamentId: tt.id, registrationId: teamBReg.id, name: 'Beta', members: { create: [{ name: 'T2a', playerId: 'T2a', memberOrder: 0 }, { name: 'T2b', playerId: 'T2b', memberOrder: 1 }] } } })
  await playoff(tt.id, 1, 0, teamAReg.id, teamBReg.id, teamAReg.id) // Alpha wins the final
  await rebuild()
  const t1aRows = await prisma.ratingLedger.findMany({ where: { tournamentId: tt.id, playerId: 'T1a' } })
  ...
  check('team NAME has no ledger rows', (await prisma.ratingLedger.count({ where: { playerId: { in: ['TEAM-ALPHA', 'TEAM-BETA'] } } })) === 0)
  const ladder = await getLadder('all-time')
  check('team members appear on the ladder', ladder.some((r) => r.playerId === 'T1a') && ladder.some((r) => r.playerId === 'T2a'))
  check('team NAMES never appear on the ladder', !ladder.some((r) => r.playerId.startsWith('TEAM-')))
  const t1aLadder = ladder.find((r) => r.playerId === 'T1a')!
  check('team champion → a trophy for every roster member', t1aLadder.trophies.length === 1 && ladder.find((r) => r.playerId === 'T1b')!.trophies.length === 1)
  check('losing team members get no trophy', ladder.find((r) => r.playerId === 'T2a')!.trophies.length === 0)
  await cleanup([tt.id])
  ...
  await rebuild()
  led = await prisma.ratingLedger.findMany({ where: { tournamentId: t1.id, playerId: 'A' }, orderBy: { sequence: 'asc' } })
  ...
  await rebuild()
  const cRowCurrent = (await getLadder('current')).find((r) => r.playerId === 'C')
  const cRowAll = (await getLadder('all-time')).find((r) => r.playerId === 'C')
  ...
  const aRow = all.find((r) => r.playerId === 'A')!
  ...
  await rebuild()
  const sA = (await getLadder('all-time')).find((r) => r.playerId === 'A')
  check('winning streak is positive and ≥6 (fire icon range)', (sA?.streak ?? 0) >= 6)
  const sB = (await getLadder('all-time')).find((r) => r.playerId === 'B')
