...
> **Season 2 launch:** the public site is now the home of **8BR Season 2** —
...
Unlike earlier phases, player pages use **real 8BR archive data as preview
...
labelled historical and are **not** presented as 8BR seasons. This is preview
...
- **Two data states** (`DataState` in mock-data): `'pending'` shows honest empty
  ...
  the components. **8BR Season 1 (`ego-season-1`) uses `'pending'`** — no invented
...
- **Historical note:** the `HistoricalNote` component (muted gold-bordered, non-
  ...
  section for 8BR Season 1 (text in `EGO_S1_HISTORICAL_NOTE`).
- **Bracket:** `PlayoffBracketShell` is a responsive shell (horizontally scrollable
  ...
  a bracket engine. 8BR Season 1 shows the pending state.
...
verification"** + **"Historical archive"** badge and are never labelled 8BR seasons.
...
`src/lib/preview-data/archive-competitions.json` (curated read-only 8BR snapshot).
...
- **Archive stats strip** — real 8BR totals from `getArchiveStats()`
  (players/aliases/matches/seasons), labelled "From the 8BR archive · preview".
...
- **Historical Title Leaders** — `TitleLeaders` (real archive championship counts);
  clearly not a current ranking ("official 8BR rankings pending"); links to
  `/players/[slug]`.
...
synthetic ranking table (dead player links) to `TitleLeaders` + an "8BR rankings
pending" note.
...
- **Indexing strategy:** `searchAll(q)` in `src/lib/search.ts` runs a straightforward
  **normalized, case-insensitive substring** match over the current preview datasets —
  players (+aliases), archive competitions, 8BR season previews, and news. No fuzzy-
...
- **Results:** grouped by type (Players · Competitions · Seasons · News); each row shows
  ...
  pending / 8BR status badge, and a link to the correct detail page.
...
- Missing data shows honest text, never fabricated content: "pending match-data import",
  "No active competition announced", "Champion pending verification", "official 8BR
  rankings pending", "Archive preview · pending verification".
- Historical archive competitions are **never** labelled 8BR seasons; the historical
  title leaderboard is **never** labelled a current 8BR ranking.
...
- Competition detail before DB: real playoff bracket rendering (results/progression)
  ...
  honest "showing X of Y" note); expand the curated set / add real 8BR competitions.
