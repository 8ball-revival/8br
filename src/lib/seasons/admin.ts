...
export async function deleteSeason(actor: Actor, seasonId: number, isHeadAdmin: boolean): Promise<{ ok: boolean; error?: string }> {
  const s = await prisma.season.findUnique({ where: { id: seasonId }, select: { number: true, year: true, lifecycleState: true } })
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await recordAudit(actor, { action: 'season.delete', entity: 'Season', entityId: seasonId, oldValue: { number: s.number, year: s.year, wasCompleted } }, tx)
