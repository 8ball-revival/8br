...
/**
 * Tournament -- the real page body, extracted so the site builder can place it.
 *
 * Moved here VERBATIM from the route: the same imports, the same reads, the same markup, the same
 * notFound() and access checks. The builder wraps this as a system module inside the `tournament`
 * template, so every Tournament page runs the genuine surface while an administrator can put content
...
export async function TournamentDetailBody({ params, searchParams }: { params: Promise<{ number: string }>; searchParams: Promise<{ view?: string }> }) {
