      ...
      >
        ...
        {input && (
          <div className="mt-4">
            ...
            {input.multiline ? (
              ...
              <input id={`${titleId}-input`} data-autofocus value={value} onChange={(e) => setValue(e.target.value)} placeholder={input.placeholder} disabled={loading} onKeyDown={(e) => { if (e.key === 'Enter' && !input.multiline) { e.preventDefault(); submit() } }} className="w-full rounded-md border border-input bg-card px-3 py-2 text-sm text-foreground outline-none focus-visible:border-brand focus-visible:ring-2 focus-visible:ring-brand/25" autoComplete="off" />
            )}
          </div>
        )}
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <button
            ...
            onClick={cancel}
            disabled={loading}
            className="rounded-md border border-input bg-card px-4 py-2 text-sm font-semibold text-muted-foreground transition-colors hover:text-foreground disabled:opacity-50"
          >
            {cancelLabel}
          </button>
          <button
