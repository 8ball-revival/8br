...
const sel = 'rounded-none border border-input bg-card px-2.5 py-1.5 text-sm outline-none focus-visible:border-brand'
...
export function CompetitionOversightView({ rows }: { rows: CompRow[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 rounded-none border border-border bg-card/40 p-3">
          ...
          <tbody className="divide-y divide-border">
            ...
            {filtered.map((r) => (
              <tr key={`${r.type}-${r.id}`} className="hover:bg-muted/40">
                <td className="px-3 py-2"><span className={cn('cyber-clip-sm px-2 py-0.5 text-[0.65rem] font-semibold', r.type === 'Season' ? 'bg-[var(--selected-surface)] text-[var(--gold)]' : 'bg-[var(--selected-surface)] text-brand')}>{r.type}</span></td>
