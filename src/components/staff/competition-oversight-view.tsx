          ...
          <tbody className="divide-y divide-border">
            ...
            {filtered.map((r) => (
              <tr key={`${r.type}-${r.id}`} className="hover:bg-muted/40">
                <td className="px-3 py-2"><span className={cn('rounded-full px-2 py-0.5 text-[0.65rem] font-semibold', r.type === 'Season' ? 'bg-[var(--selected-surface)] text-[var(--gold)]' : 'bg-[var(--selected-surface)] text-brand')}>{r.type}</span></td>
