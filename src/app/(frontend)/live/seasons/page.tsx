...
import { getLiveSeasons } from '@/lib/competition/surface'
...
export const metadata: Metadata = pageMetadata({
  title: 'Live Seasons',
  description: 'Seasons currently under way at the 8 Ball Registry.',
  path: '/live/seasons',
})

/** Seasons under way. Viewing only — every management control lives in Creator. */
export default async function LiveSeasonsPage() {
  const cards = await getLiveSeasons()
  return (
    <Wide name="live-seasons" className="py-6">
      <h1 className="font-display text-2xl font-bold tracking-tight sm:text-3xl">Live Seasons</h1>
      <p className="mt-1 text-sm text-muted-foreground">
        Seasons with registration open or play under way.
      </p>

      {cards.length === 0 ? (
        <p className="mt-8 rounded-lg border border-border bg-card p-8 text-center text-sm text-muted-foreground">
          No Seasons are running right now. Completed Seasons are in the{' '}
          <a href="/archives/seasons" className="text-[var(--gold)] hover:underline">Archives</a>.
        </p>
      ) : (
